        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <Card variant="glass" className="max-w-2xl w-full p-8 shadow-lg rounded-lg">
            ...
            <div className="space-y-4">
              <div>
                <label className="block font-semibold mb-2 text-foreground/90 tracking-wide">Post Type *</label>
                <div className="grid grid-cols-3 gap-3">
                  {POST_TYPES.map((type) => (
                    <button
                      key={type.id}
                      onClick={() => setSelectedPostType(type.id)}
                      className={`p-4 border-2 rounded-lg text-center transition-all duration-500 ${selectedPostType === type.id ? 'border-primary/40 bg-primary/15 backdrop-blur-xl' : 'border-white/15 bg-white/5 hover:border-white/25 hover:bg-white/8'}`}
                    >
              ...
              <div>
                <label className="block font-semibold mb-2 text-foreground/90 tracking-wide">Title *</label>
                <Input placeholder="What's the title?" value={newPost.title} onChange={(e) => setNewPost(p => ({ ...p, title: e.target.value }))} />
              </div>

              {selectedPostType === 'COLLAB' && (
                <div>
                  <label className="block font-semibold mb-2 text-foreground/90 tracking-wide">Pod Name * <span className="text-xs text-muted-foreground/50">(Name for the collaboration room)</span></label>
                  <Input placeholder="e.g., 'AI Research Collab'" value={newPost.podName} onChange={(e) => setNewPost(p => ({ ...p, podName: e.target.value }))} />
                </div>
              )}

              <div>
                <label className="block font-semibold mb-2 text-foreground/90 tracking-wide">Content / Description</label>
                <Textarea placeholder="What are the details?" value={newPost.content} onChange={(e) => setNewPost(p => ({ ...p, content: e.target.value }))} />
              </div>

              {selectedPostType === 'POLL' && (
                <div>
                  <label className="block font-semibold mb-2 text-foreground/90 tracking-wide">Poll Options *</label>
              ...
              {selectedPostType !== 'POLL' && (
                <div>
                  <label className="block font-semibold mb-2 text-foreground/90 tracking-wide">Tags</label>
                  <div className="flex flex-wrap gap-2 mb-2">
                    {skillTags.map((tag) => (
                      <Badge
                        key={tag}
                        variant="secondary"
                        className="cursor-pointer hover:bg-red-500/10 transition-colors duration-300 rounded-lg"
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-between pt-4 mt-4 border-t border-slate-700">
                  ...
                  {post.type === 'POLL' ? (
                    ...
                    <Button onClick={() => navigate(`/post/${post.id}/comments`, { state: { from: '/campus', sourceView: 'inter', sourceContext: 'inter-feed' } })} variant="outline" size="sm" className="bg-slate-700/50 border-slate-600 hover:bg-slate-700">Reply</Button>
                  )}
                </div>
              </CardContent>
            </Card>
